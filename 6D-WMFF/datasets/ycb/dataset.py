...
class PoseDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.add_noise:
            ...
            nm_in = self.trancolor(nm_in)

        rmin, rmax, cmin, cmax = get_bbox(mask_label)
        img = np.transpose(np.array(img)[:, :, :3], (2, 0, 1))[:, rmin:rmax, cmin:cmax]
        nm_in = np.transpose(np.array(nm_in)[:, :, :3], (2, 0, 1))[:, rmin:rmax, cmin:cmax]
        ...
        img_mask = np.transpose(mask_crop,(1,2,0))
        nm_in = np.transpose(nm_in,(1,2,0))
        img_mask_for_depth = img_mask[:,:,0]
        rgb_crop = cv2.bitwise_and(img_mask,img_rgb)
        nm_in_crop = cv2.bitwise_and(img_mask,nm_in)
        ...
        if self.list[index][:8] == 'data_syn':
            rgb_crop = rgb_crop + np.random.normal(loc=0.0, scale=7.0, size=rgb_crop.shape)
        ...
        return torch.from_numpy(cloud.astype(np.float32)), \
               ...
               self.norm(torch.from_numpy(rgb_crop.astype(np.float32))), \
               torch.from_numpy(target.astype(np.float32)), \
               torch.from_numpy(model_points.astype(np.float32)), \
               torch.LongTensor([int(obj[idx]) - 1]), \
               torch.from_numpy(mask_crop.astype(np.float32)/255.0), \
               self.norm(torch.from_numpy(nm_in_crop.astype(np.float32)))
