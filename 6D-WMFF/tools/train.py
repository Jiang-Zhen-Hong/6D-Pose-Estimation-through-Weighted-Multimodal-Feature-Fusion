...
parser.add_argument('--nepoch', type=int, default= 500, help='max number of epochs to train[50]')
    ...
    for epoch in range(opt.start_epoch, opt.nepoch):
        ...
        for rep in range(opt.repeat_epoch):
            for i, data in enumerate(dataloader, 0):
                points, choose, img, target, model_points, idx, mask, nm_in = data
                points, choose, img, target, model_points, idx, mask, nm_in = Variable(points).cuda(), \
                ...
                pred_r, pred_t, pred_c, emb, img_f = estimator(img, points, choose, idx, mask,nm_in)
                ...
                if opt.refine_start:
                    for ite in range(0, opt.iteration):
                        pred_r, pred_t = refiner(new_points, emb, idx, img_f)
