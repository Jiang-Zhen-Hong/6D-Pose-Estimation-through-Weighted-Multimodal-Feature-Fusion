...
for now in range(0, 2949):
    ...
    my_result_wo_refine = []
    my_result = []
    nm_in = normalSpeed.depth_normal(depth.astype(np.uint16), cam_fx, cam_fy, k_size, distance_threshold, difference_threshold, True)
    ...
    for idx in range(len(lst)):
        ...
        try:
            ...
            nm_in = np.array(nm_in)[:, :, :3]
            nm_in = np.transpose(nm_in, (2, 0, 1))
            nm_in = nm_in[:, rmin:rmax, cmin:cmax]
            ...
            my_mask = mask_label.copy()
            mask_crop = my_mask[rmin:rmax, cmin:cmax]
            mask_crop = mask_crop.astype(int)
            mask_crop = np.reshape(mask_crop, (mask_crop.shape[0], mask_crop.shape[1], -1))
            mask_crop[mask_crop > 0] = 255
            mask_crop = cv2.cvtColor(mask_crop.astype(np.uint8), cv2.COLOR_GRAY2RGB)
            

            img_rgb = np.transpose(img_masked,(1,2,0))
            nm_in = np.transpose(nm_in,(1,2,0))
            rgb_crop = cv2.bitwise_and(mask_crop,img_rgb)
            nm_in_crop = cv2.bitwise_and(mask_crop,nm_in)

            rgb_crop = np.transpose(rgb_crop,(2,0,1))
            nm_in_crop = np.transpose(nm_in_crop, (2,0,1))
            mask_crop = np.transpose(mask_crop, (2,0,1))

            rgb_crop = norm(torch.from_numpy(rgb_crop.astype(np.float32)))
            nm_in_crop = norm(torch.from_numpy(nm_in_crop.astype(np.float32)))
            mask_crop = torch.from_numpy(mask_crop.astype(np.float32))/255

            rgb_crop = Variable(rgb_crop).cuda()
            nm_in_crop = Variable(nm_in_crop).cuda()
            mask_crop = Variable(mask_crop).cuda()

            rgb_crop = rgb_crop.view(1, 3, rgb_crop.size()[1], rgb_crop.size()[2])
            nm_in_crop = nm_in_crop.view(1, 3, nm_in_crop.size()[1], nm_in_crop.size()[2])
            mask_crop = mask_crop.view(1, 3, mask_crop.size()[1], mask_crop.size()[2])

            pred_r, pred_t, pred_c, emb, img_f = estimator(rgb_crop, cloud, choose, index, mask_crop, nm_in_crop)
            ...
            for ite in range(0, iteration):
                ...
                pred_r, pred_t = refiner(new_cloud, emb, index, img_f)
